...
print("LangCell Transcriptome Tokenization Pipeline - Kidney Cancer Binary Classification")
print("==================================================================================")
...
for col in adata.obs.columns:
    if col in ['sample', 'cell_type', 'source', 'clusters_by_authors']:
        obs2col[col] = col
...
if unassigned_mask.sum() > 0:
    ...
    if 'mp_assignment' in adata.obs.columns:
        ...
        # Map MP assignments to cell types for kidney cancer based on actual data
        mp_to_celltype = {
            'Stress': 'Malignant',
            'Cilia': 'Epithelial',
...
if 'clusters_by_authors' in adata.obs.columns:
    cluster_dist = adata.obs['clusters_by_authors'].value_counts()
...
dataset_name = f"tokenized_dataset_kidney_binary_{timestamp}"
...
binary_types = {
    ...
    'Non-malignant': 'Non-malignant cells including normal epithelial, immune cells, stromal cells, and other non-cancerous cell types in kidney tissue'
}

# Save binary classification descriptions
import json
binary_type2text_path = "/cs/student/projects1/aibh/2024/rmaheswa/Dissertation/LangCell/your_data/binary_type2text_kidney.json"
...
print("\n" + "="*70)
print("KIDNEY CANCER BINARY CLASSIFICATION PREPROCESSING COMPLETE!")
print("="*70)
