...
print("LangCell Transcriptome Tokenization Pipeline - Prostate Cancer Binary Classification")
print("==================================================================================")

# 1) Load AnnData
adata_path = "/cs/student/projects1/aibh/2024/rmaheswa/Dissertation/data/prostate_cancer_with_celltypes_v3.h5ad"
...
if converted_count == 0:
    ...
    print("Sample gene names from prostate data (cleaned):")
...
# 4) Build maps - comprehensive for prostate_cancer_with_celltypes_v3.h5ad
# Map the available columns to appropriate names for LangCell
obs2col = {}
for col in adata.obs.columns:
    if col in ['sample', 'cell_type', 'source', 'group']:
        obs2col[col] = col
...
if unassigned_mask.sum() > 0:
    ...
    if 'mp_assignment' in adata.obs.columns:
        ...
        # Map MP assignments to cell types for prostate cancer
        mp_to_celltype = {
...
if 'sample_age' in adata.obs.columns:
    age_dist = adata.obs['sample_age'].value_counts()
    print(f"  Age distribution:")
    for age, count in age_dist.items():
        percentage = (count / len(adata.obs)) * 100
        print(f"    Age {age}: {count} cells ({percentage:.1f}%)")

if 'sample_sex' in adata.obs.columns:
    sex_dist = adata.obs['sample_sex'].value_counts()
    print(f"  Sex distribution:")
    for sex, count in sex_dist.items():
        percentage = (count / len(adata.obs)) * 100
        print(f"    {sex}: {count} cells ({percentage:.1f}%)")

if 'sample_patient' in adata.obs.columns:
    patient_dist = adata.obs['sample_patient'].value_counts()
    print(f"  Patients: {len(patient_dist)} unique patients")
    for patient, count in patient_dist.items():
        percentage = (count / len(adata.obs)) * 100
        print(f"    Patient {patient}: {count} cells ({percentage:.1f}%)")

if 'sample_histology' in adata.obs.columns:
    histology_dist = adata.obs['sample_histology'].value_counts()
    print(f"  Histology distribution:")
    for histology, count in histology_dist.items():
        percentage = (count / len(adata.obs)) * 100
        print(f"    {histology}: {count} cells ({percentage:.1f}%)")

if 'sample_AJCC_stage' in adata.obs.columns:
    stage_dist = adata.obs['sample_AJCC_stage'].value_counts()
    print(f"  AJCC Stage distribution:")
    for stage, count in stage_dist.items():
        percentage = (count / len(adata.obs)) * 100
        print(f"    {stage}: {count} cells ({percentage:.1f}%)")

if 'sample_smoking_status' in adata.obs.columns:
    smoking_dist = adata.obs['sample_smoking_status'].value_counts()
...
dataset_name = f"tokenized_dataset_prostate_binary_{timestamp}"
...
binary_types = {
    'Malignant': 'Malignant epithelial cells from prostate cancer, showing abnormal proliferation and cancer-specific gene expression patterns',
    'Non-malignant': 'Non-malignant cells including normal epithelial, immune cells, stromal cells, and other non-cancerous cell types in prostate tissue'
}

# Save binary classification descriptions
import json
binary_type2text_path = "/cs/student/projects1/aibh/2024/rmaheswa/Dissertation/LangCell/your_data/binary_type2text_prostate.json"
...
cell_types = {
    ...
    'Epithelial': 'Normal epithelial cells in prostate tissue, involved in prostate function and tissue structure',
    'T_cell': 'T lymphocytes in prostate tissue, involved in adaptive immune response and immune surveillance',
    'Endothelial': 'Endothelial cells lining blood vessels in prostate tissue, involved in vascular function and angiogenesis',
    'Fibroblast': 'Fibroblast cells in prostate tissue, involved in extracellular matrix production and tissue remodeling',
    'Macrophage': 'Macrophage immune cells in prostate tissue, involved in immune response and tissue homeostasis',
    'Mast': 'Mast cells in prostate tissue, involved in inflammatory responses and immune regulation',
    'B_cell': 'B lymphocytes in prostate tissue, involved in antibody production and humoral immune response'
}
...
print("\n" + "="*70)
print("PROSTATE CANCER BINARY CLASSIFICATION PREPROCESSING COMPLETE!")
print("="*70)
